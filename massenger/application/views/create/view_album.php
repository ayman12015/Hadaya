...
<div class="container gallery">
    ...
    if (isset($picturelist) && !empty($picturelist)) {
        foreach($picturelist as $key => $value) { ?>            
                <ul class="thumbnails">
                    <li class="span3">
                            <a class="thumbnail" rel="prettyPhoto[pp_gal]" href="<?php echo base_url().'uploads/gallery/'.$value->image?>" title="<?php echo $value->title?>"><img src="<?php echo base_url().'uploads/gallery/'.$value->image?>" alt="" /></a>
                        </li> <!--end thumb -->
                </ul><!--end thumbnails -->
        <?php } 
    } else { ?>
